	...
	function createAfterHook(operation) {
		const afterHook = function afterHook(instance, opt) {
			...
			if (
				...
				console.log(
					'is this also the userkey',
					query[options.userModelAttribute],
				);

				const revision = Revision.build(query);
				console.log('WHAT IS THE userId', opt.userId, revision);
				if (query[options.userModelAttribute]) {
					revision.dataValues[options.userModelAttribute] =
						query[options.userModelAttribute];
					console.log(
						'saved the user key/iud',
						query[options.userModelAttribute],
						options.userModelAttribute,
					);
				}
				...
				return revision
					...
					.then((objectRevision) => {
						...
						// 	query[options.userModelAttribute]
						// ) {
						// 	return objectRevision[`set${options.userModel}`](
						// 		query[options.userModelAttribute],
						// 	);
						// }
